...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_model = random.choice(['SM-S911B', 'ASUS_I005DA', 'LG-D802TR', 'Nokia 5 V', 'TECNO LD7', 'GT-I9060M', 'E2303', 'GT-H8570', 'vivo X20Plus UD'])
            self.useragent = ('Mozilla/5.0 (Linux; Android {}; {}) AppleWebKit/537.36 AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{} iThunder;thirdChannel_SHOUJIXUNLEI/7.24.2.7543 xl_cloud statusBarHeight/72'.format(self.android_version, self.android_model, self.build, self.browser_version))
