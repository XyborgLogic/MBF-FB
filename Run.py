...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_model = random.choice(['Nokia 2 V', 'Nokia G11 Plus', 'LG-D802TR', 'Nokia 5 V', 'N150DL', 'Nokia G22', 'TA-1021', 'TA-1032', 'Nokia 3 V '])
            self.useragent = ('Mozilla/5.0 (Linux; Android {}; {}) Build/PKQ1.181105.001) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{} Mobile Safari/537.36 Hawk/QuickBrowser/2.4.23.115507'.format(self.android_version, self.android_model, self.build, self.browser_version))
