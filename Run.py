...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_version = random.choice(['8.1.0','7.1.2','7.0','12'])
            self.android_model = random.choice(['Redmi 5', 'Redmi 4X', 'Redmi Note 4X','ASUS_I005DA'])
            self.useragent = ('Dalvik/2.1.0 (Linux; Android {}; {} Build/N2G47H; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{} Mobile Safari/537.36 [FB_IAB/Orca-Android;FBAV/291.0.0.20.114;]'.format(self.android_version, self.android_model, self.build, self.browser_version))
